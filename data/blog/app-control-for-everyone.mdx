...
Let's have a look at `fapolicyd` on Red Hat Enterprise Linux (you can get a copy at [developers.redhat.com](https://developers.redhat.com) The package is available with Red Hat Enterprise Linux 8.2+, and can be installed through the AppStream repositories.
```
$ sudo dnf install fapolicyd fapolicyd-cli
$ sudo systemctl start fapolicyd
```
If an application is installed through RPM, it's fine to use:
```
