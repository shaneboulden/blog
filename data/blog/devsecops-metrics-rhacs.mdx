...
These metrics don't directly answer risk and security questions, though it is implicit. If we patch a critical CVE this is captured in the deployment frequency and lead time for changes, and maybe also the time to restore the service. I'd like to propose another metric though that more explicitly looks at risk - **Critical CVE detection frequency**. This is the number of critical CVEs detected throughout the application lifecycle. 
