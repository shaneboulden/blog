...
We can also directly query the Rekor transparency ledger for the in-toto attestation that's been created. Firstly we need to search for any transparency log entries using the image digest.
```
