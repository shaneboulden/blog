...
Ok - let's get hands-on! You can follow the [Sigstore docs](https://docs.sigstore.dev/cosign/installation/) to grab a copy of the `cosign`, `rekor` and `fulcio` binaries.

You can use Fulcio-issues certificates to sign containers by simply using `cosign sign`. I'm going to sign the image `quay.io/smileyfritz/chat-client`. Note that you require access to the registry / repository for the image you want to sign, as the signature is pushed to the registry and stored as an OCI object.
...
```
cosign sign quay.io/smileyfritz/rfiweb@sha256:86c9473bdae1201f870c4a328b07bcba5e18a6b4e83c26669cf82a36d7b8cbd5
```
...
```
We can also directly query the Rekor transparency ledger for the attestation that's been created. Firstly we need to search for any transparency log entries using the image digest.
```
...
...
```
And that's it! We've signed a container with `cosign` using certificates and keys signed by the Fulcio CA, and created attestations and entries in the Rekor transparency ledger. If you want to see some of the other signing workflows available, there's an excellent in-depth tutorial available at [chainguard.dev](https://edu.chainguard.dev/open-source/sigstore/cosign/how-to-sign-a-container-with-cosign/).
...
2. Configure Tekton Chains. You can find a quick guide [here](https://docs.openshift.com/container-platform/4.11/cicd/pipelines/using-tekton-chains-for-openshift-pipelines-supply-chain-security.html). Make sure that you patch the Tekton Chains config:
