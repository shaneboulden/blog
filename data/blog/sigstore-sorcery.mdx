...
Importantly, rekor is also extensible. 
...
Fulcio has also recently reached General Availability. The Sigstore community maintaining the "public good" Sigstore instance provides a 99.5% Availability service level objective (SLO), and uses [semver](link to semver) rules for API stability.
...
That's really all there is. You don't need to explicitly add steps to your build pipeline to sign containers - the Tekton Chains controller will detect new releases from the `IMAGE_URL` and `IMAGE_DIGEST` results, sign these containers with the key provided, and store the signatures as OCI manifest objects alongside your container images in the registry.
