...
So what is [llm-d](https://llm-d.ai/)? One of the challenges with generative AI inference is that everything happens on the same GPU, on the same server. This makes it difficult to scale resources needed to support AI platforms. llm-d solves these challenges by leveraging distributed inference optimisations, like KV-cache aware routing and disaggregated serving. You can find a video here showing llm-d in action:

<YoutubeEmbed embedId="pYujrc3rGjk" />

You can also read a blog [here](https://liming.dev/articles/2025-05-25-llmd/) about why llm-d is a gamechanger for AI inference from one of Red Hat's AI experts in Singapore, Li Ming Tsai. Li Ming and I both joined Red Hat in the same month back in 2016, and were both part of the professional services organisation working on the same product, [Red Hat CloudForms](https://www.redhat.com/en/technologies/management/cloudforms).
...
The first event on the Trailblazer agenda was a tour of Fenway park. It was facinating hearing the history of the stadium and the Boston Red Sox, including the background to the ["green monster"](https://en.wikipedia.org/wiki/Green_Monster) and why the "monster seats" are so expensive (~ USD $1000 per game!).
...
Part of the award was a dinner with Trailblazer award winners and Red Hat leaders, including Paul Cormier himself. I had an awesome time speaking with the other Red Hatters, and hearing their stories of innovation, and creating solutions to really interesting problems. [Richard Chun](https://www.linkedin.com/in/richardchun) also graciously snapped a pic of me with the Red Hat President and CEO, Matt Hicks.
