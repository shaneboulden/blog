---
title: Functional Verification with Ansible
date: '2022-09-30'
...
---
Over the last couple of weeks I've had conversations around 'functional verification'. Essentially functional verification extends IT system comppliance checks by functionally verifying whether the configuration is correct.
...
Functional verification extends these compliance checks by testing the configuration. In this example we've checked that `fapolicyd` is enabled and started, and the configuration looks ok. We can functionally verify this configuration by:
...
- If the file successfully executes, the playbook fails.
...
The binary we pulled from GitHub was able to successfull execute (we can see parts of the `cowsay` cow in the output), so the playbook fails. Now what happens when we start `fapolicyd` and run the playbook:
...
Success! The playbook finished successfully - which means that the file was blocked from executing, which we can see in the debug statement (`"cowsay_cmd.rc": "1"`).

## Next steps

In this article I've quickly looked at functional verification with Ansible, using the `failed_when` test to control failure. There's a couple of ways you may want to extend this:

- Reporting functional test failures into a SIEM
