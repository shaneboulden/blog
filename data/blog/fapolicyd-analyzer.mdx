...
For this write-up I'm going to install the latest version of `fapolicy-analyzer` on Fedora 36. Firstly, we need some dependencies:
```
...
Let's try profiling an application. I've created a new user on the system and pulled down a binary `cowsay` we can run locally. If you navigate to 'Tools -> Profile Executable' you can specify these parameters:
...
This example is a little more interesting. In addition to the `/usr/local/bin/ansible` binary, we have a number of shared libraries the application attempted to access. This is certainly useful for understanding how complex applications access system components, and helping create exceptions.
